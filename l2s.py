#  LOGS -> statsd, в конфиге - работающий пример для главного балансировщика ясеня
...
def format_statsd(message, pattern):
    return pattern['statsd'].format(**message).encode('utf8')
    ...
    for mask, path, line in multi_follow(patterns.keys(), jump=jump, follow=follow):
        ...
        for pattern in patterns[mask]:
            ...
            if match:
                ...
                sender(format_statsd(message, pattern), (host, port))
                #print(format_statsd(message, pattern), (host, port))
                #break
